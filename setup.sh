...
# ==============================================================================
#             VODKA GATEWAY PREMIUM INTERACTIVE SETUP WIZARD
# ==============================================================================
# A professional, high-end, highly stylized interactive CLI automation tool
# for provisioning, deploying, and destroying Project VODKA on Cloudflare Edge.
...
EOF
    echo -e "${NC}"
    echo -e "${CYAN}┌────────────────────────────────────────────────────────────────────────┐${NC}"
    echo -e "${CYAN}│${NC}               ${BOLD}VODKA Edge Gateway Installer — Premium Edition${NC}          ${CYAN}│${NC}"
    echo -e "${CYAN}└────────────────────────────────────────────────────────────────────────┘${NC}"
}
...
install_VODKA() {
    ...
    if run_with_spinner " ${INFO} Requesting Cloudflare API to provision D1 Database '$DB_NAME'..." npx wrangler d1 create "$DB_NAME"; then
        D1_OUTPUT=$(cat /tmp/VODKA_cmd.log)
        ...
        if echo "$D1_OUTPUT" | grep -qE "already exists|already_exists" 2>/dev/null; then
            ...
            if run_with_spinner " ${INFO} Retrieving available D1 database details..." npx wrangler d1 list --json; then
                D1_LIST_OUTPUT=$(cat /tmp/VODKA_cmd.log)
    ...
    echo -e " ${ASK} Define a name for your Cloudflare Edge Worker [Default: VODKA-core]: "
    read -p " ❯ " WORKER_NAME
    WORKER_NAME=${WORKER_NAME:-VODKA-core}
...
# Production wrangler.toml compiled automatically by VODKA Setup Script
name = "$WORKER_NAME"
main = "_worker.js"
...
EOF
    ...
    if run_with_spinner " ${INFO} Uploading scripts, linking bindings, and activating Edge nodes..." npx wrangler deploy; then
        DEPLOY_OUTPUT=$(cat /tmp/VODKA_cmd.log)
    ...
    if [ -z "$DEPLOY_URL" ]; then
        ...
        echo -e " ${ASK} Please enter your Worker Domain manually (e.g. VODKA.username.workers.dev):"
        read -p " ❯ " USER_URL
    ...
    echo -e " ${OK} Press [Enter] to access the VODKA Control Dashboard..."
    read -r
...
EOF
    ...
    console.log(cyan + '┌' + '─'.repeat(76) + '┐' + nc);
    console.log(padLine('          🚀   CONGRATULATIONS! VODKA EDGE GATEWAY IS ONLINE!   🚀        '));
    console.log(cyan + '├' + '─'.repeat(76) + '┤' + nc);
...
uninstall_VODKA() {
    clear
    show_header
    echo -e "\n${BOLD}${RED}─── [ DESTRUCTION WIZARD ] REMOVE VODKA FROM CLOUDFLARE ───${NC}\n"
    ...
    console.log(cyan + '├' + '─'.repeat(76) + '┤' + nc);
    console.log(padLine('This wizard will permanently tear down your VODKA Worker, drop the'));
    ...
    echo -e " ${ASK} Are you absolutely sure you want to completely wipe VODKA from Cloudflare? (y/n)"
    ...
    if [ -z "$WORKER_NAME" ]; then
        ...
        echo -e " ${ASK} Please input the Cloudflare Worker name to destroy [Default: VODKA-core]:"
        read -p " ❯ " WORKER_NAME
        WORKER_NAME=${WORKER_NAME:-VODKA-core}
    else
...
EOF
    ...
    console.log(cyan + '┌' + '─'.repeat(76) + '┐' + nc);
    console.log(padLine('💥  ' + red + bold + 'VODKA UNINSTALLATION OPERATIONS SUMMARY' + nc));
    console.log(cyan + '├' + '─'.repeat(76) + '┤' + nc);
    console.log(padLine(
...
main_menu() {
    while true; do
        ...
        console.log(cyan + '├' + '─'.repeat(76) + '┤' + nc);
        console.log(padLine(''));
        console.log(padLine('  ' + green + '1)' + nc + '  🚀  Install / Deploy VODKA Project to Cloudflare Edge'));
        console.log(padLine('  ' + red + '2)' + nc + '  💀  Uninstall / Wipe VODKA Project from Cloudflare'));
        ...
        case "$choice" in
            ...
            3)
                clear
                echo -e "\n ${OK} Thank you for using VODKA Gateway Installer. Safe travels! 👋\n"
                exit 0
                ;;
            *)
